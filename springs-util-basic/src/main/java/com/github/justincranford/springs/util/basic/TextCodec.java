...
public interface TextEncDec {
	public static final HexEncDec HEX_UC_STRICT  = new HexEncDec(true,  true,  "HexUpperCaseStrict");
	public static final HexEncDec HEX_LC_STRICT  = new HexEncDec(false, true,  "HexLowerCaseStrict");
	public static final HexEncDec HEX_UC_LENIENT = new HexEncDec(true,  false, "HexUpperCaseLenient");
	public static final HexEncDec HEX_LC_LENIENT = new HexEncDec(false, false, "HexLowerCaseLenient");
	public static final Base64EncDec BASE64_STD    = new Base64EncDec(Constants.STD_ENCODER,    Constants.STD_DECODER,  "Base64");
	public static final Base64EncDec BASE64_URL    = new Base64EncDec(Constants.URL_ENCODER,    Constants.URL_DECODER,  "Base64URL");
	public static final Base64EncDec BASE64_MIME32 = new Base64EncDec(Constants.MIME32_ENCODER, Constants.MIME_DECODER, "Base64Mime32");
	public static final Base64EncDec BASE64_MIME64 = new Base64EncDec(Constants.MIME64_ENCODER, Constants.MIME_DECODER, "Base64Mime64");
	public static final Base64EncDec BASE64_MIME76 = new Base64EncDec(Constants.MIME76_ENCODER, Constants.MIME_DECODER, "Base64Mime76");
	public static final Base64EncDec BASE64_MIME   = new Base64EncDec(Constants.MIME_ENCODER,   Constants.MIME_DECODER, "Base64Mime");
	...
	public static class HexEncDec implements TextEncDec {
    	...
    	/*package*/ HexEncDec(final boolean encodeUpperCase0, final boolean decodeStrict0, final String canonicalId) {
    	...
    	@Override
        public String encodeToString(final byte[] bytes) {
    		final byte[] hex = encodeToBytes(Objects.requireNonNull(bytes, "Bytes must be non-null"));
			return new String(hex, 0, hex.length, StandardCharsets.UTF_8);
    	}
    	@Override
        public byte[] encodeToBytes(final byte[] bytes) {
    		final byte[] encodedBytes = new byte[Objects.requireNonNull(bytes, "Bytes must be non-null").length * 2];
    	...
    	@Override
        public byte[] decodeFromString(final String string) {
    		return decodeFromBytes(Objects.requireNonNull(string, "String must be non-null").getBytes(StandardCharsets.UTF_8));
    	}
    	@Override
        public byte[] decodeFromBytes(final byte[] bytes) {
            if (Objects.requireNonNull(bytes, "Bytes must be non-null").length % 2 != 0) {
	...
	public static class Base64EncDec implements TextEncDec {
    	private final Base64.Encoder encoder;
    	private final Base64.Decoder decoder;
    	private final byte[] canonicalIdBytes;
    	/*package*/ Base64EncDec(final Base64.Encoder encoder0, final Base64.Decoder decoder0, final String canonicalId) {
