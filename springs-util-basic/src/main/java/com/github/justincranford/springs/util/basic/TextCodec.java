...
public interface TextCodec {
    HexCodec HEX_UC_STRICT = new HexCodec(true, true, "HexUpperCaseStrict");
    HexCodec HEX_UC_LENIENT = new HexCodec(true, false, "HexUpperCaseLenient");
    HexCodec HEX_LC_STRICT = new HexCodec(false, true, "HexLowerCaseStrict");
    HexCodec HEX_LC_LENIENT = new HexCodec(false, false, "HexLowerCaseLenient");

    Base64Codec B64_STD = new Base64Codec(Constants.STD_ENCODER, Constants.STD_DECODER, "Base64");
    Base64Codec B64_URL = new Base64Codec(Constants.URL_ENCODER, Constants.URL_DECODER, "Base64URL");
    Base64Codec B64_MIME32 = new Base64Codec(Constants.MIME32_ENCODER, Constants.MIME_DECODER, "Base64Mime32");
    Base64Codec B64_MIME64 = new Base64Codec(Constants.MIME64_ENCODER, Constants.MIME_DECODER, "Base64Mime64");
    Base64Codec B64_MIME76 = new Base64Codec(Constants.MIME76_ENCODER, Constants.MIME_DECODER, "Base64Mime76");
    Base64Codec B64_MIME = new Base64Codec(Constants.MIME_ENCODER, Constants.MIME_DECODER, "Base64Mime");

    byte[] canonicalIdBytes();

    String encodeToString(final byte[] bytes);

    byte[] encodeToBytes(final byte[] bytes);

    byte[] decodeFromString(final String string);

    byte[] decodeFromBytes(final byte[] bytes);
