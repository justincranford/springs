...
@Configuration
@Import({KeyValueEncodersConfiguration.KeyEncodersConfiguration.class, KeyValueEncodersConfiguration.ValueEncodersConfiguration.class})
@SuppressWarnings({"deprecation", "nls"})
public class KeyValueEncodersConfiguration {
	// TODO upgradeEncoding true
	private static final PasswordEncoder NOOP_PASSWORD_ENCODER = NoOpPasswordEncoder.getInstance();

	@Configuration
	public final class KeyEncodersConfiguration {
		...
		public IdAndPasswordEncoder keyEncoder1() {
			...
			return new IdAndPasswordEncoder("1", new EncodersArgon2.DerivedSalt(contextBytes, minimumDerivedSaltLength, 32, 1, 1 << 14, 2));
		}
		// @VisibleForTesting
		public IdAndPasswordEncoder keyEncoder0() {
			...
			return new IdAndPasswordEncoder("0", new EncodersArgon2.ConstantSalt(contextBytes, constantSaltBytes, 16, 1, 1 << 13, 1));
		}
	}

	@Configuration
	public final class ValueEncodersConfiguration {
		...
		public IdAndPasswordEncoder valueEncoder1() {
			final byte[] contextBytes = "context1".getBytes();
			return new IdAndPasswordEncoder("1", new EncodersArgon2.RandomSalt(contextBytes, 16, 32, 1, 1 << 14, 2));
		}
		// @VisibleForTesting
		public IdAndPasswordEncoder valueEncoder0() {
			final byte[] contextBytes = "context0".getBytes();
			return new IdAndPasswordEncoder("0", new EncodersArgon2.RandomSalt(contextBytes, 16, 32, 1, 1 << 14, 2));
		}
	}
