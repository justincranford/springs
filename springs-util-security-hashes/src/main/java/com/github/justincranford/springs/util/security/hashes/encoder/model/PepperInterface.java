...
public interface PepperInterface {
    static byte[] safeComputeAndEncode(@Nullable PepperInterface pepperInterface, @NotEmpty final byte[] rawInput, @NotNull final byte[] additionalData) {
    ...
    @NotNull
    MacAlgorithm mac();                // required (e.g. HmacSHA256, CMAC256); used as Mac digest, as well as for deriving low-entropy hmacKey if secretKey=null

    @NotNull
    TextCodec codec();                // required (e.g. mitigate bcrypt truncation weaknesses w.r.t null bytes and max 72-bytes)

    @Nullable
    SecretKey secretKey();                // high-entropy 256-bit random key; n.b. may be null

    @Nullable
    DigestAlgorithm secretKeyDigest();    // if secretKey omitted, required to derive secretKeyBytes from inputs

    @NotNull
    byte[] secretContext();            // may be empty (e.g. any-entropy N-byte value)

    @NotNull
    byte[] clearContext();                // may be empty (e.g. "application1".getBytes(), "feature1".getBytes(), any-entropy N-byte value)
}
