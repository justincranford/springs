...
public record Pepper(
	...
	@Nullable SecretKey secretKey,				// high-entropy 256-bit random key; n.b. may be null
	@Nullable DigestAlgorithm secretKeyDigest,	// if secretKey omitted, required to derive secretKeyBytes from inputs
	...
	public static int safeLength(@Nullable final PepperInterface pepper, @Min(1) final int defaultLen) {
		return (pepper != null) ? pepper.mac().outputBytesLen() : defaultLen;
	}

	public static byte[] safeDecode(@Nullable final PepperInterface pepper, @Min(1) final byte[] bytes) {
