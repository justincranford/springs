...
public interface PersonOrmRepository extends ListCrudRepository<PersonOrm, Long>, RevisionRepository<PersonOrm, Long, Long> {
    ...
    @Modifying
    @Query("UPDATE PersonOrm p SET p.password.password=:password, p.lastModifiedDate=:now, p.preUpdateDateTime=:now, p.postUpdateDateTime=:now, p.version=p.version+1" +
    		" WHERE p.id=:id")
