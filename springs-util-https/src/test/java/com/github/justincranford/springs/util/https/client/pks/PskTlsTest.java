...
//				Thread.sleep(100); // waiting for server to call serverSocket.accept() doesn't seem to help
                ...
                log.info("Main: Waited for Server start for " + (System.nanoTime() - nanos) / 1000000F + " msec");
            }
            return serverThread;
        }
    ...
    public static class TlsPskIdentity implements TlsPSKIdentity {
        ...
        @Override
        public void skipIdentityHint() { /*do nothing*/ }

        @Override
        public void notifyIdentityHint(byte[] psk_identity_hint) { /*do nothing*/ }
    ...
    public static class TlsPskIdentityManager implements TlsPSKIdentityManager {
        ...
        public TlsPskIdentityManager(final TlsPskIdentity _tlsPskIdentity) {
            this.tlsPskIdentity = _tlsPskIdentity;
        }

        @Override
        public byte[] getHint() {
            return this.tlsPskIdentity.getPSKIdentity();
        }

        @Override
        public byte[] getPSK(byte[] identity) {
            return this.tlsPskIdentity.getPSK();
        }
    }
}
